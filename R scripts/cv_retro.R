...
ssb_tmp <- getSSB(df.tmb, sas)
cv_est[i+1] <- ssb_tmp$SE[nrow(ssb_tmp)]
cv_adv[i+1] <- ssb_tmp$SE[nrow(ssb_tmp)-1]
}


yrtmp <- rev(years[(nyear-npeels+1):nyear])

plot(yrtmp, cv_est,type='l', ylim = c(0.1,.4))
lines(yrtmp, cv_adv, col = 'red')
