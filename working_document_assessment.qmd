...
quarterly_mr <- read.table('four_quarters/mohns_table.csv', header = TRUE) %>% mutate(model = 'quarter')
hy_mr <- read.table('two_seasons/mohns_table.csv', header = TRUE) %>% mutate(model = 'half year')
# Read the mohns rho calculations 
...
mr.plot$label <- sprintf("rho_SSB = %.3f\nrho_R = %.3f\nrho_F = %.3f",
                         mr.plot$SSB, mr.plot$R, mr.plot$F0)
...
p.mr.ssb <- ggplot(df.plot, aes(x = years, y = SSB, color = factor(peel)))+
  ...
  geom_text(
    data = mr.plot,
    aes(
      x = 2001,   # tweak as needed
      y = Inf,    # top of each panel
      label = label
    ),
    vjust = 1.2,
    hjust = 0,
...
p.mr.R <- ggplot(df.plot, aes(x = years, y = R, color = factor(peel)))+
  ...
  geom_text(
    data = mr.plot,
    aes(
      x = 2001,          # choose a fixed x-position within your xlim
    ...
    size = 3,
    inherit.aes = FALSE,
    parse = TRUE        # enables Greek rho + subscript
  )
