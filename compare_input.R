...
wd <- "C:/Users/nsja/Dropbox/DTU/BEBRIS/old_method/"
...
old <- readRDS('2025_assessment.RDS')

M2 <- old$dat$M

x <- getM(old$dat) %>% mutate(model = 'old')
xnew <- getM(df.tmb) %>% mutate(model = 'new')

m2plot <- rbind(x,xnew)


ggplot(m2plot %>% filter(age == 0), 
       aes(x = years, y = value, color = model))+geom_line()+facet_wrap(~season)

# Check weca 

weca_old <- getWeight(old$dat)%>% mutate(model = 'old')
weca_new <- getWeight(df.tmb) %>% mutate(model = 'new')
weca <- rbind(weca_old, weca_new)

ggplot(weca, 
       aes(x = years, y = value, color = factor(age), linetype = model))+geom_line()+facet_wrap(~season)
