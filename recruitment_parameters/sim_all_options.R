...
saveRDS(results_list, file = 'recruitment_parameters/all_sims_two.RDS')


getAssess <- function(dats){
...
mr <- mohns_rho(df.tmb, parms,plotfigure = FALSE)

# Save stuff 
out <- getSummary(df.tmb, sas)
info <- list(mohns = mr$mohns,
             parameters = dats,
            convergence = sas$reps$pdHess,
            AIC = AIC(sas))
