...
dat <- getDataSMS(wd,
  ...
  surveyEnd =  c(1,1,0.2) # c(1,0) Does the survey last throughout the season it's conducted?
...
df.tmb <- get_TMB_parameters(
  ...
  maxSDcatch = sqrt(2),
  Fbarage = c(1,2),
  isFseason = c(1,1,0,0), # Seasons to calculate fishing in
  powers = powerIN,
...
library(tidyverse)
x <- compareTMB_ADMB(wd, sas, df.tmb)
